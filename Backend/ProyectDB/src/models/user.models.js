...
const updateUser = async (id, user) => {
  let { nombre, apellido, email, password, nombre_usuario, puntos_xp, nivel } = user;

  if (!password) {
    const sql = "UPDATE users SET nombre = ?, apellido = ?, email = ?, nombre_usuario=?, puntos_xp=?, nivel=? WHERE idUsers = ?";
    const [result] = await pool.query(sql, [nombre, apellido, email, nombre_usuario, puntos_xp, nivel, id]);
    return result;
  } else {
    const bcrypt = require("bcrypt");
    const hashed = bcrypt.hashSync(password, 10);
    const sql = "UPDATE users SET nombre = ?, apellido = ?, email = ?, password = ?, nombre_usuario=?, puntos_xp=?, nivel=? WHERE idUsers = ?";
    const [result] = await pool.query(sql, [nombre, apellido, email, hashed, nombre_usuario, puntos_xp, nivel, id]);
    return result;
  }
};
